import {
	GET_POSTS, GET_POSTS_SUCCESS, GET_POSTS_FAILURE, RESET_POSTS,
	GET_POST, GET_POST_SUCCESS,  GET_POST_FAILURE, RESET_ACTIVE_POST,
...
export default function posts(state = INITIAL_STATE, action) {
  ...
  case GET_POSTS_BY_CATEGORY_SUCCESS:// return list of posts and make loading = false
    return { ...state, postsList: {posts: action.payload, error:null, loading: false} };
  case GET_POSTS_BY_CATEGORY_FAILURE:// return error and make loading = false
    error = action.payload || {message: action.payload.message};//2nd one is network or server down errors
    return { ...state, postsList: {posts: [], error: error, loading: false} };
  case RESET_POSTS:// reset postList to initial state
    return { ...state, postsList: {posts: [], error:null, loading: false} };
